...
,  const char* format
   ...
   ;  thedata = wrap_sw_fill(&ai, left, right, g_indel_allowed, swp)
   ...
   ;  if (format)
      sw_format(annot_left, annot_right, cell, &ai, fpo, 0, recno, NULL, format)
...
;if(0)fprintf(stderr, "recs %d %d %d %d\n", rs->first_kmer_position, rs->last_kmer_independent_pos, rs->last_kmer_pos, rs->n_kmer_independent_hits)
      ...
      ;  if (id_threshold)
         {  if (id_threshold && ai.aln_identity >= id_threshold)
            ...
            ;  if (!g_grep)
               do_align(fpo, rs->s, rs->annot, thequery->s, thequery->annot, swp, todo[i], NULL)
...
/* exit macromagicalitaciousness */
   ...
   ;  if (fpref)
      ...
      ;  q = ls_query
      ;  while (q->s)
         {  best_i = search_best_match(fpo, &ref_index, &swp, ls_ref, q, &overhang, todo, n_todo, theidentity, &n_found)
...
;if (best_i >= 0 && !n_found) die(1, "impossible (%d)", best_i)
         ;  if (g_grep)
            {  if (n_found && (g_grep & GREP_MATCH))
               do_align(fpo, ls_ref[best_i].s, ls_ref[best_i].annot, q->s, q->annot, &swp, best_i, g_format_grep)
            ...
            {  do_align(fpo, ls_ref[best_i].s, ls_ref[best_i].annot, q->s, q->annot, &swp, best_i, NULL)
         ;  }
            q++
   ...
   ;  }                             /* hierverder: no fpquery + single reference apparently */
      ...
      do_align(fpo, singleref, "reference", singlequery ? singlequery : singleref, "query", &swp, 1, NULL)
   ;  else
      die(1, "no reference!")
