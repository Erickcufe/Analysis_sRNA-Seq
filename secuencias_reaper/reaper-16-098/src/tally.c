...
,  void* fpo
,  int zippit
,  unsigned lr             /* left/right bits; if 0 then there is no second output stream */
,  const char* q
)
   {  char output[MAXFIELDSIZE+1]
   ;  const char* f = format
...
puts("  Paired-end reads (activated with -j option) can be output with -o alone or both -o and -p");
